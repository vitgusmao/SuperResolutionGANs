...
def ResDenseBlock_5C(block_input, nf=64, gc=32, res_beta=0.2, wd=0.0, name="RDB5C"):
    ...
    conv1 = _Conv2DLayer(filters=gc, activation=lrelu_f())
    conv2 = _Conv2DLayer(filters=gc, activation=lrelu_f())
    conv3 = _Conv2DLayer(filters=gc, activation=lrelu_f())
    conv4 = _Conv2DLayer(filters=gc, activation=lrelu_f())
    conv5 = _Conv2DLayer(filters=nf, activation=lrelu_f())

    x1 = conv1(block_input)
    x2 = conv2(tf.concat([block_input, x1], 3))
    x3 = conv3(tf.concat([block_input, x1, x2], 3))
    x4 = conv4(tf.concat([block_input, x1, x2, x3], 3))
    x5 = conv5(tf.concat([block_input, x1, x2, x3, x4], 3))
    return x5 * res_beta + block_input


def ResInResDenseBlock(block_input, nf=64, gc=32, res_beta=0.2, wd=0.0, name="RRDB"):
    rdb_1 = ResDenseBlock_5C(block_input, nf, gc, res_beta=res_beta, wd=wd)
    rdb_2 = ResDenseBlock_5C(rdb_1, nf, gc, res_beta=res_beta, wd=wd)
    rdb_3 = ResDenseBlock_5C(rdb_2, nf, gc, res_beta=res_beta, wd=wd)

    return rdb_3 * res_beta + block_input
...
def RRDB_Model(
    ...
    fea_rrdb = fea
    for i in range(nb):
        fea_rrdb = rrdb_f(block_input=fea, name="RRDB_{}".format(i))
