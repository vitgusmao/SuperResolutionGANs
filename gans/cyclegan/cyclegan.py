...
def get_discriminator(filters=64,
                      kernel_initializer=kernel_init,
                      num_downsampling=3,
                      name=None):
    img_input = layers.Input(shape=input_img_size, name=name + "_img_input")
...
class CycleGan(keras.Model):
    ...
    def train_step(self, batch_data):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            cycle_loss_G = self.cycle_loss_fn(real_y,
                                              cycled_y) * self.lambda_cycle
            cycle_loss_F = self.cycle_loss_fn(real_x,
                                              cycled_x) * self.lambda_cycle
...
class GANMonitor(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        for i, img in enumerate(test_horses.take(self.num_img)):
            ...
            prediction.save("generated_img_{i}_{epoch}.png".format(
                i=i, epoch=epoch + 1))
        plt.show()
        plt.close()
...
cycle_gan_model = CycleGan(generator_G=gen_G,
                           generator_F=gen_F,
